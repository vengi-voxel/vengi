...
void SceneManager::construct() {
	const core::VarDef voxEditColorWheel(cfg::VoxEditColorWheel, false, -1, _("Use the color wheel in the palette color editing"));
	core::Var::registerVar(voxEditColorWheel);
	const core::VarDef voxEditShowColorPicker(cfg::VoxEditShowColorPicker, false, -1, _("Always show the color picker below the palette"));
	core::Var::registerVar(voxEditShowColorPicker);
	const core::VarDef voxEditModificationDismissMillis(cfg::VoxEditModificationDismissMillis, "1500", -1, _("Milliseconds that a region should get highlighted in a few situations"));
	core::Var::registerVar(voxEditModificationDismissMillis);
	const core::VarDef voxEditRegionSizes(cfg::VoxEditRegionSizes, "", -1, _("Show fixed region sizes in the volume inspector"), core::Var::ivec3ListValidator<1, 256>);
	core::Var::registerVar(voxEditRegionSizes);
	const core::VarDef voxEditLocalSpace(cfg::VoxEditLocalSpace, true, -1, _("Use local space for transforms"));
	core::Var::registerVar(voxEditLocalSpace);
	const core::VarDef voxEditShowgrid(cfg::VoxEditShowgrid, true, -1, _("Show the grid"));
	core::Var::registerVar(voxEditShowgrid);
	const core::VarDef voxEditShowlockedaxis(cfg::VoxEditShowlockedaxis, true, -1, _("Show the currently locked axis"));
	core::Var::registerVar(voxEditShowlockedaxis);
	const core::VarDef voxEditShowaabb(cfg::VoxEditShowaabb, true, -1, _("Show the axis aligned bounding box"));
	core::Var::registerVar(voxEditShowaabb);
	const core::VarDef voxEditShowBones(cfg::VoxEditShowBones, false, -1, _("Show the bones in scene mode"));
	core::Var::registerVar(voxEditShowBones);
	const core::VarDef voxEditRendershadow(cfg::VoxEditRendershadow, false, -1, _("Render with shadows - make sure to set the scene lighting up properly"));
	core::Var::registerVar(voxEditRendershadow);
	const core::VarDef voxEditShadingMode(cfg::VoxEditShadingMode, 1, -1, _("Shading mode: 0=Unlit (pure colors), 1=Lit (no shadows), 2=Shadows"), core::Var::minMaxValidator<0, 2>);
	core::Var::registerVar(voxEditShadingMode);
	const core::VarDef voxEditAnimationSpeed(cfg::VoxEditAnimationSpeed, 100, -1, _("Millisecond delay between frames hide/unhide when using the scene graph panel play button to animate the models in the scene"));
	core::Var::registerVar(voxEditAnimationSpeed);
	const core::VarDef voxEditAutoNormalMode(cfg::VoxEditAutoNormalMode, 0, core::CV_NOPERSIST, "Flat, Smooth, Smoother", core::Var::minMaxValidator<0, 2>);
	core::Var::registerVar(voxEditAutoNormalMode);
	const core::VarDef voxEditGridsize(cfg::VoxEditGridsize, 1, -1, _("The size of the voxel grid"), core::Var::minMaxValidator<1, 64>);
	core::Var::registerVar(voxEditGridsize);
	const core::VarDef voxEditPlaneSize(cfg::VoxEditPlaneSize, 100, -1, _("The size of the plane"), core::Var::minMaxValidator<1, 1000>);
	core::Var::registerVar(voxEditPlaneSize);
	const core::VarDef voxEditShowPlane(cfg::VoxEditShowPlane, true, -1, _("Show the plane"));
	core::Var::registerVar(voxEditShowPlane);
	const core::VarDef voxEditGrayInactive(cfg::VoxEditGrayInactive, false, -1, _("Render the inactive nodes in gray scale mode"));
	core::Var::registerVar(voxEditGrayInactive);
	const core::VarDef voxEditHideInactive(cfg::VoxEditHideInactive, false, -1, _("Hide the inactive nodes"));
	core::Var::registerVar(voxEditHideInactive);
	const core::VarDef voxEditViewdistance(cfg::VoxEditViewdistance, 5000, -1, _("Far plane for the camera"));
	core::Var::registerVar(voxEditViewdistance);
	const core::VarDef voxEditShowaxis(cfg::VoxEditShowaxis, true, -1, _("Show the axis"));
	core::Var::registerVar(voxEditShowaxis);
	const core::VarDef voxEditCursorDetails(cfg::VoxEditCursorDetails, 1, -1, _("Print cursor details in edit mode - measure distance to reference position in mode 3"), core::Var::minMaxValidator<0, 3>);
	core::Var::registerVar(voxEditCursorDetails);
	const core::VarDef voxEditAutoKeyFrame(cfg::VoxEditAutoKeyFrame, true, -1, _("Automatically create keyframes when changing transforms"));
	core::Var::registerVar(voxEditAutoKeyFrame);
	const core::VarDef voxEditGizmoOperations(cfg::VoxEditGizmoOperations, 3, -1, _("Bitmask of gizmo operations in scene mode"));
	core::Var::registerVar(voxEditGizmoOperations);
	const core::VarDef voxEditGizmoPivot(cfg::VoxEditGizmoPivot, false, core::CV_NOPERSIST, _("Activate the pivot mode for the gizmo in scene mode"));
	core::Var::registerVar(voxEditGizmoPivot);
	const core::VarDef voxEditGizmoAllowAxisFlip(cfg::VoxEditGizmoAllowAxisFlip, true, -1, _("Flip axis or stay along the positive world/local axis"));
	core::Var::registerVar(voxEditGizmoAllowAxisFlip);
	const core::VarDef voxEditGizmoSnap(cfg::VoxEditGizmoSnap, true, -1, _("Use the grid size for snap"));
	core::Var::registerVar(voxEditGizmoSnap);
	const core::VarDef voxEditModelGizmo(cfg::VoxEditModelGizmo, false, -1, _("Show the gizmo to also translate the region"));
	...
	const core::VarDef voxEditViewports(cfg::VoxEditViewports, 2, -1, _("The amount of viewports (not in simple ui mode)"), core::Var::minMaxValidator<2, cfg::MaxViewports>);
	core::Var::registerVar(voxEditViewports);
	const core::VarDef voxEditMaxSuggestedVolumeSize(cfg::VoxEditMaxSuggestedVolumeSize, 128, -1, _("The maximum size of a volume before a few features are disabled (e.g. undo/autosave)"), core::Var::minMaxValidator<32, voxedit::MaxVolumeSize>);
	core::Var::registerVar(voxEditMaxSuggestedVolumeSize);
	const core::VarDef voxEditViewMode(cfg::VoxEditViewMode, "default", -1, _("Configure the editor view mode"));
	core::Var::registerVar(voxEditViewMode);
	const core::VarDef voxEditTipOftheDay(cfg::VoxEditTipOftheDay, true, -1, _("Show the tip of the day on startup"));
	core::Var::registerVar(voxEditTipOftheDay);
	const core::VarDef voxEditPopupTipOfTheDay(cfg::VoxEditPopupTipOfTheDay, false, core::CV_NOPERSIST, _("Trigger opening of popup"));
	core::Var::registerVar(voxEditPopupTipOfTheDay);
	const core::VarDef voxEditPopupWelcome(cfg::VoxEditPopupWelcome, false, core::CV_NOPERSIST, _("Trigger opening of popup"));
	core::Var::registerVar(voxEditPopupWelcome);
	const core::VarDef voxEditPopupMinecraftMapping(cfg::VoxEditPopupMinecraftMapping, false, core::CV_NOPERSIST, _("Trigger opening of popup"));
	core::Var::registerVar(voxEditPopupMinecraftMapping);
	const core::VarDef voxEditPopupAbout(cfg::VoxEditPopupAbout, false, core::CV_NOPERSIST, _("Trigger opening of popup"));
	core::Var::registerVar(voxEditPopupAbout);
	const core::VarDef voxEditPopupRenameNode(cfg::VoxEditPopupRenameNode, false, core::CV_NOPERSIST, _("Trigger opening of popup"));
	core::Var::registerVar(voxEditPopupRenameNode);
	const core::VarDef voxEditPopupCreateAnimation(cfg::VoxEditPopupCreateAnimation, false, core::CV_NOPERSIST, _("Trigger opening of popup"));
	core::Var::registerVar(voxEditPopupCreateAnimation);

	const core::VarDef voxEditAnimationPlaying(cfg::VoxEditAnimationPlaying, false, core::CV_NOPERSIST, _("Update the children of a node when the transform of the node changes"));
	core::Var::registerVar(voxEditAnimationPlaying);
	const core::VarDef voxEditAutoSaveSeconds(cfg::VoxEditAutoSaveSeconds, 180, -1, _("Delay in second between autosaves - 0 disables autosaves"));
	_autoSaveSecondsDelay = core::Var::registerVar(voxEditAutoSaveSeconds);
	const core::VarDef voxEditTransformUpdateChildren(cfg::VoxEditTransformUpdateChildren, true, -1, _("Update the children of a node when the transform of the node changes"));
	...
	command::Command::registerCommand("palette_changeintensity")
		...
		.addArg({"nodeid", command::ArgType::String, true, "", "Node ID to apply the intensity change to"})
		.setHandler([&] (const command::CommandArgs& args) {
	...
	command::Command::registerCommand("palette_warmer")
		...
		.addArg({"nodeid", command::ArgType::String, true, "", "Node ID to apply the warmth to"})
		.setHandler([&] (const command::CommandArgs& args) {
			const int nodeId = toNodeId(args, activeNode());
			scenegraph::SceneGraphNode &node = _sceneGraph.node(nodeId);
			palette::Palette &pal = node.palette();
	...
	command::Command::registerCommand("palette_colder")
		...
		.addArg({"nodeid", command::ArgType::String, true, "", "Node ID to apply the coldness to"})
		.setHandler([&] (const command::CommandArgs& args) {
			const int nodeId = toNodeId(args, activeNode());
			scenegraph::SceneGraphNode &node = _sceneGraph.node(nodeId);
			palette::Palette &pal = node.palette();
	...
	command::Command::registerCommand("palette_brighter")
		...
		.addArg({"nodeid", command::ArgType::String, true, "", "Node ID to apply the brightness to"})
		.setHandler([&] (const command::CommandArgs& args) {
			const int nodeId = toNodeId(args, activeNode());
			scenegraph::SceneGraphNode &node = _sceneGraph.node(nodeId);
			palette::Palette &pal = node.palette();
	...
	command::Command::registerCommand("palette_darker")
		...
		.addArg({"nodeid", command::ArgType::String, true, "", "Node ID to apply the darkness to"})
		.setHandler([&] (const command::CommandArgs& args) {
			const int nodeId = toNodeId(args, activeNode());
			scenegraph::SceneGraphNode &node = _sceneGraph.node(nodeId);
			palette::Palette &pal = node.palette();
	...
	command::Command::registerCommand("palette_removeunused")
		...
		.addArg({"nodeid", command::ArgType::String, true, "", "Node ID to remove unused colors from"})
		.setHandler([&] (const command::CommandArgs& args) {
	...
	command::Command::registerCommand("palette_whitebalancing")
		.addArg({"nodeid", command::ArgType::String, true, "", "Node ID to apply the white balance to"})
		.setHandler([&] (const command::CommandArgs& args) {
			const int nodeId = toNodeId(args, activeNode());
			scenegraph::SceneGraphNode &node = _sceneGraph.node(nodeId);
	...
	command::Command::registerCommand("palette_contraststretching")
		.addArg({"nodeid", command::ArgType::String, true, "", "Node ID to apply the contrast stretching to"})
		.setHandler([&] (const command::CommandArgs& args) {
			const int nodeId = toNodeId(args, activeNode());
			scenegraph::SceneGraphNode &node = _sceneGraph.node(nodeId);
			palette::Palette currentPal = node.palette();
	...
	command::Command::registerCommand("palette_applyall")
		.addArg({"nodeid", command::ArgType::String, true, "", "Node ID to apply the palette from"})
		.setHandler([&] (const command::CommandArgs& args) {
			const int nodeId = toNodeId(args, activeNode());
	...
	command::Command::registerCommand("palette_sort")
		...
		.addArg({"nodeid", command::ArgType::String, true, "", "Node ID to apply the sort to"})
		.setHandler([&] (const command::CommandArgs& args) {
	...
	command::Command::registerCommand("normpalette_removenormals")
		.addArg({"nodeid", command::ArgType::String, true, "", "Node ID to remove the normals from"})
		.setHandler([&] (const command::CommandArgs& args) {
	...
	command::Command::registerCommand("crop")
		.addArg({"nodeid", command::ArgType::String, true, "", "Node ID to crop"})
		.setHandler([&] (const command::CommandArgs& args) {
			const int nodeId = toNodeId(args, activeNode());
			crop(nodeId);
	...
	command::Command::registerCommand("colortomodel")
		...
		.addArg({"nodeid", command::ArgType::String, true, "", "Node ID to create"})
		.setHandler([&] (const command::CommandArgs& args) {
			const int nodeId = toNodeId(args, activeNode());
			if (args.has("index")) {
	...
	command::Command::registerCommand("loadnormalpalette")
		...
		.addArg({"nodeid", command::ArgType::String, true, "", "Node ID to change"})
		.setHandler([this] (const command::CommandArgs& args) {
			const int nodeId = toNodeId(args, activeNode());
			scenegraph::SceneGraphNode *node = sceneGraphNode(nodeId);
			if (node == nullptr) {
	...
	command::Command::registerCommand("copy")
		.addArg({"nodeid", command::ArgType::String, true, "", "Node ID to copy from"})
		.setHandler([&] (const command::CommandArgs& args) {
	...
	command::Command::registerCommand("pastenewnode")
		.addArg({"nodeid", command::ArgType::String, true, "", "Parent node ID"})
		.setHandler([&] (const command::CommandArgs& args) {
			pasteAsNewNode(toNodeId(args, activeNode()));
		}).setHelp(_("Paste clipboard as a new node"));

	command::Command::registerCommand("cut")
		.addArg({"nodeid", command::ArgType::String, true, "", "Node ID to cut from"})
		.setHandler([&] (const command::CommandArgs& args) {
	...
	command::Command::registerCommand("modelmerge")
		.addArg({"nodeid1", command::ArgType::String, true, "", "First node ID"})
		.addArg({"nodeid2", command::ArgType::String, true, "", "Second node ID"})
		.setHandler([&] (const command::CommandArgs& args) {
			int nodeId1;
			int nodeId2;
	...
	command::Command::registerCommand("nodedelete")
		.addArg({"nodeid", command::ArgType::String, true, "", "Node ID to delete"})
		.setHandler([&] (const command::CommandArgs& args) {
			const int nodeId = toNodeId(args, activeNode());
			if (scenegraph::SceneGraphNode* node = sceneGraphNode(nodeId)) {
	...
	command::Command::registerCommand("nodelock")
		.addArg({"nodeid", command::ArgType::String, true, "", "Node ID to lock"})
		.setHandler([&] (const command::CommandArgs& args) {
			const int nodeId = toNodeId(args, activeNode());
			if (scenegraph::SceneGraphNode* node = sceneGraphNode(nodeId)) {
				node->setLocked(true);
	...
	command::Command::registerCommand("nodeunlock")
		.addArg({"nodeid", command::ArgType::String, true, "", "Node ID to unlock"})
		.setHandler([&] (const command::CommandArgs& args) {
			const int nodeId = toNodeId(args, activeNode());
			if (scenegraph::SceneGraphNode* node = sceneGraphNode(nodeId)) {
	...
	command::Command::registerCommand("nodeactivate")
		.addArg({"nodeid", command::ArgType::String, true, "", "Node ID to activate"})
		.setHandler([&] (const command::CommandArgs& args) {
			if (!args.has("nodeid")) {
	...
	command::Command::registerCommand("nodehideothers")
		.addArg({"nodeid", command::ArgType::String, true, "", "Node ID to keep visible"})
		.setHandler([&] (const command::CommandArgs& args) {
			const int nodeId = toNodeId(args, activeNode());
			for (auto iter = _sceneGraph.beginAll(); iter != _sceneGraph.end(); ++iter) {
				scenegraph::SceneGraphNode &node = *iter;
	...
	command::Command::registerCommand("nodeduplicate")
		.addArg({"nodeid", command::ArgType::String, true, "", "Node ID to duplicate"})
		.setHandler([&] (const command::CommandArgs& args) {
			const int nodeId = toNodeId(args, activeNode());
			if (scenegraph::SceneGraphNode *node = sceneGraphNode(nodeId)) {
				nodeDuplicate(*node);
	...
	command::Command::registerCommand("modelref")
		.addArg({"nodeid", command::ArgType::String, true, "", "Node ID to reference"})
		.setHandler([&] (const command::CommandArgs& args) {
			const int nodeId = toNodeId(args, activeNode());
			if (scenegraph::SceneGraphNode *node = sceneGraphNode(nodeId)) {
				nodeReference(*node);
	...
	command::Command::registerCommand("modelunref")
		.addArg({"nodeid", command::ArgType::String, true, "", "Node ID to unreference"})
		.setHandler([&] (const command::CommandArgs& args) {
			const int nodeId = toNodeId(args, activeNode());
			if (scenegraph::SceneGraphNode *node = sceneGraphNode(nodeId)) {
	...
	command::Command::registerCommand("cam_activate")
		.addArg({"nodeid", command::ArgType::String, true, "", "Camera node ID"})
		.setHandler([&] (const command::CommandArgs& args) {
