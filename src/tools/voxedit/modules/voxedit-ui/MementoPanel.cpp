...
static inline core::String toString(const MementoState &state, const core::String &name, int n) {
	return core::string::format("%s (%s): node %i, parent %i, keyframe: %u, name: %s##%i",
								MementoHandler::typeToString(state.type), name.c_str(), state.nodeId, state.parentId,
...
void MementoPanel::update(const char *id, command::CommandExecutionListener &listener) {
	...
	if (ImGui::Begin(title.c_str(), nullptr, ImGuiWindowFlags_NoFocusOnAppearing)) {
		...
		if (ImGui::BeginListBox("##history-actions", ImVec2(-FLT_MIN, -FLT_MIN))) {
			struct State {
				State(const core::String *name, const MementoState *state, int stateIdx) : name(name), state(state), stateIdx(stateIdx) {}
				const core::String *name;
				const MementoState *state;
				const int stateIdx;
			};
			core::DynamicArray<State> states;
			states.reserve(mementoHandler.stateSize());
			int stateIdx = 0;
			for (const MementoStateGroup &group : mementoHandler.states()) {
