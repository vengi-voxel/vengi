...
static glm::ivec3 createBezierTrunk(Volume& volume, const voxelgenerator::TreePalm& ctx, const voxel::Voxel& voxel) {
	...
	shape::createBezierFunc(volume, ctx.pos, end, control, voxel,
		[&] (Volume& in, const glm::ivec3& last, const glm::ivec3& pos, const voxel::Voxel& v) {
			shape::createLine(in, pos, last, v, core_max(1, (int)glm::ceil(trunkSize)));
...
void createTreePalm(Volume& volume, const voxelgenerator::TreePalm& ctx, math::Random& random, const voxel::Voxel &trunkVoxel, const voxel::Voxel &leavesVoxel) {
	...
	for (int b = 0; b < ctx.branches; ++b) {
		...
		shape::createBezierFunc(volume, start, end, control, leavesVoxel,
			[&] (Volume& in, const glm::ivec3& last, const glm::ivec3& pos, const voxel::Voxel& voxel) {
				// TODO: this should be some kind of polygon - not a line - we want a flat leaf
				shape::createLine(in, pos, last, voxel, core_max(1, (int)glm::ceil(branchSize)));
 ...
 */
template<class Volume>
void createTree(Volume& volume, const palette::Palette &palette, const voxelgenerator::TreeContext& ctx, math::Random& random) {
	const uint8_t green = palette.getClosestMatch(color::RGBA(123, 162, 63));
	const uint8_t brown = palette.getClosestMatch(color::RGBA(143, 90, 60));
