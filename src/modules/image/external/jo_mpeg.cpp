...
// Huffman tables
static const unsigned char s_jo_HTDC_Y[9][2] = {{4, 3},	 {0, 2},  {1, 2},  {5, 3},	{6, 3},
												{14, 4}, {30, 5}, {62, 6}, {126, 7}};
static const unsigned char s_jo_HTDC_C[9][2] = {{0, 2},	 {1, 2},  {2, 2},	{6, 3},	 {14, 4},
												{30, 5}, {62, 6}, {126, 7}, {254, 8}};
static const unsigned char s_jo_HTAC[32][40][2] = {
	{
		{6, 3},	  {8, 5},	{10, 6},  {12, 8},	{76, 9},  {66, 9},	{20, 11}, {58, 13}, {48, 13}, {38, 13},
		{32, 13}, {52, 14}, {50, 14}, {48, 14}, {46, 14}, {62, 15}, {62, 15}, {58, 15}, {56, 15}, {54, 15},
		{52, 15}, {50, 15}, {48, 15}, {46, 15}, {44, 15}, {42, 15}, {40, 15}, {38, 15}, {36, 15}, {34, 15},
		{32, 15}, {48, 16}, {46, 16}, {44, 16}, {42, 16}, {40, 16}, {38, 16}, {36, 16}, {34, 16}, {32, 16},
	},
	{{6, 4},
	 {12, 7},
	 {74, 9},
...
static const float s_jo_quantTbl[64] = {
	0.015625f, 0.005632f, 0.005035f, 0.004832f, 0.004808f, 0.005892f, 0.007964f, 0.013325f, 0.005632f, 0.004061f,
	0.003135f, 0.003193f, 0.003338f, 0.003955f, 0.004898f, 0.008828f, 0.005035f, 0.003135f, 0.002816f, 0.003013f,
	0.003299f, 0.003581f, 0.005199f, 0.009125f, 0.004832f, 0.003484f, 0.003129f, 0.003348f, 0.003666f, 0.003979f,
	0.005309f, 0.009632f, 0.005682f, 0.003466f, 0.003543f, 0.003666f, 0.003906f, 0.004546f, 0.005774f, 0.009439f,
	0.006119f, 0.004248f, 0.004199f, 0.004228f, 0.004546f, 0.005062f, 0.006124f, 0.009942f, 0.008883f, 0.006167f,
	0.006096f, 0.005777f, 0.006078f, 0.006391f, 0.007621f, 0.012133f, 0.016780f, 0.011263f, 0.009907f, 0.010139f,
	0.009849f, 0.010297f, 0.012133f, 0.019785f,
};
static const unsigned char s_jo_ZigZag[] = {0,	1,	5,	6,	14, 15, 27, 28, 2,	4,	7,	13, 16, 26, 29, 42,
...
static void stream_putc(uint8_t c, io::WriteStream *fp) {
	fp->write(&c, 1);
}
...
static void stream_fwrite(const char *data, int size, int count, io::WriteStream *fp) {
	fp->write(data, size * count);
}
...
bool jo_write_mpeg(io::WriteStream *fp, const uint8_t *rgbx, int width, int height, int fps) {
	...
	stream_putc((width >> 4) & 0xFF, fp);
	stream_putc(((width & 0xF) << 4) | ((height >> 8) & 0xF), fp);
	stream_putc(height & 0xFF, fp);
	...
	if (fps <= 24)
		...
		stream_putc(0x18, fp); // 60fps
	stream_fwrite("\xFF\xFF\xE0\xA0", 4, 1, fp);
	...
	stream_fwrite("\x00\x00\x01\xb7", 4, 1, fp); // End of Sequence
	return true;
}
