...
TEST_F(ClipperTest, testClippingBlockedRight) {
	const glm::vec3 worldPos(0.5f, 0.0f, 0.0f);
	const glm::vec3 dir(1.0f, 0.0f, 0.0f);
	const glm::mat3 noRot(1.0f);
	voxelutil::RaycastResult result = _clipper.clipDelta(_sceneGraph, InvalidFrame, worldPos, dir, noRot);
	EXPECT_TRUE(result.isInterrupted()) << "Clipping should detect the solid voxel and prevent movement";
}
...
TEST_F(ClipperTest, testClippingNoBlockedRightHalf) {
	...
	const glm::mat3 noRot(1.0f);
	voxelutil::RaycastResult result = _clipper.clipDelta(_sceneGraph, InvalidFrame, worldPos, dir, noRot);
	EXPECT_TRUE(result.isCompleted()) << "Clipping should allow movement if it does not cross the solid voxel boundary";
}
...
TEST_F(ClipperTest, testClippingNoBlockedRight) {
	...
	const glm::mat3 noRot(1.0f);
	voxelutil::RaycastResult result = _clipper.clipDelta(_sceneGraph, InvalidFrame, worldPos, dir, noRot);
	EXPECT_TRUE(result.isCompleted()) << "Clipping should not detect any solid voxel and allow movement";
}
...
TEST_F(ClipperTest, testClippingBlockedTop) {
	const glm::vec3 worldPos(1.0f, -0.9f, 0.0f);
