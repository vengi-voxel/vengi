...
bool TBWidgetSkinConditionContext::getCondition(TBSkinCondition::TARGET target,
	...
	case TBSkinCondition::TARGET_PARENT:
		return (m_widget->getParent() != nullptr) && getCondition(m_widget->getParent(), info);
	...
	case TBSkinCondition::TARGET_PREV_SIBLING:
		return (m_widget->getPrev() != nullptr) && getCondition(m_widget->getPrev(), info);
	case TBSkinCondition::TARGET_NEXT_SIBLING:
		return (m_widget->getNext() != nullptr) && getCondition(m_widget->getNext(), info);
	}
	return false;
}
...
bool TBWidgetSkinConditionContext::getCondition(TBWidget *widget, const TBSkinCondition::CONDITION_INFO &info) {
	...
	case TBSkinCondition::PROPERTY_STATE:
		return !((widget->getAutoState() & info.value) == 0U);
	case TBSkinCondition::PROPERTY_VALUE:
		return widget->getValue() == (int)info.value;
	case TBSkinCondition::PROPERTY_HOVER:
		return (TBWidget::hovered_widget != nullptr) && widget->isAncestorOf(TBWidget::hovered_widget);
	case TBSkinCondition::PROPERTY_CAPTURE:
		return (TBWidget::captured_widget != nullptr) && widget->isAncestorOf(TBWidget::captured_widget);
	case TBSkinCondition::PROPERTY_FOCUS:
		return (TBWidget::focused_widget != nullptr) && widget->isAncestorOf(TBWidget::focused_widget);
