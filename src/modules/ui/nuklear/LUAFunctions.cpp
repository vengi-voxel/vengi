...
static float floatsArray[NK_LOVE_MAX_RATIOS];
...
static void uilua_assert(lua_State* s, bool pass, const char *msg) {
	if (pass) {
		return;
	}
	lua_Debug ar;
...
static inline void uilua_assert_argc(lua_State* s, bool pass) {
	uilua_assert(s, pass, "wrong number of arguments to '%s'");
}
...
static void uilua_color(int r, int g, int b, int a, char *colorString) {
	r = NK_CLAMP(0, r, 255);
	g = NK_CLAMP(0, g, 255);
	b = NK_CLAMP(0, b, 255);
	a = NK_CLAMP(0, a, 255);
...
static struct nk_image uilua_checkImage(lua_State *s, int index) {
	...
	uilua_assert(s, texturePool != nullptr, "Could not get texture pool: '%s'");
	const video::TexturePtr& tex = texturePool->load(imageName);
	uilua_assert(s, tex != nullptr, "Could not load image: '%s'");
...
int uilua_window_begin(lua_State *s) {
	...
	if (noNameDefined) {
		uilua_assert_argc(s, lua_gettop(s) >= 5);
		name = title = luaL_checkstring(s, 1);
		startIndex = 2;
	} else {
		uilua_assert_argc(s, lua_gettop(s) >= 6);
...
int uilua_window_end(lua_State *s) {
	uilua_assert_argc(s, lua_gettop(s) == 0);
	struct nk_context* ctx = uilua_ctx(s);
	nk_end(ctx);
	return 0;
}
...
int uilua_button(lua_State *s) {
	const int argc = lua_gettop(s);
	uilua_assert_argc(s, argc >= 1 && argc <= 2);
	struct nk_context* ctx = uilua_ctx(s);
	...
	if (title != nullptr) {
		if (useColor) {
			uilua_assert(s, false, "%s: color buttons can't have titles");
		...
		if (useColor) {
			...
			uilua_assert(s, false, "%s: must specify a title, color, symbol, and/or image");
		}
	}
...
int uilua_push_scissor(lua_State *s) {
	uilua_assert_argc(s, lua_gettop(s) == 4);
	struct nk_context* ctx = uilua_ctx(s);
...
int uilua_label(lua_State *s) {
	...
	uilua_assert_argc(s, argc >= 1 && argc <= 3);
	struct nk_context* ctx = uilua_ctx(s);
...
int uilua_text(lua_State *s) {
	...
	const nk_color& color = uilua_checkcolor(s, 6);
	nk_draw_text(&ctx->current->buffer, nk_rect(x, y, w, h), text, strlen(text), ctx->style.font, nk_rgba(0, 0, 0, 0), color);
	return 0;
}

int uilua_image(lua_State *s) {
	const int argc = lua_gettop(s);
	uilua_assert_argc(s, argc == 5);
	struct nk_context* ctx = uilua_ctx(s);
	...
	const float w = luaL_checknumber(s, 4);
	const float h = luaL_checknumber(s, 5);
	nk_draw_image(&ctx->current->buffer, nk_rect(x, y, w, h), &image, nk_color{255, 255, 255, 255});
	return 0;
}
...
int uilua_window_is_collapsed(lua_State *s) {
	struct nk_context* ctx = uilua_ctx(s);
	uilua_assert_argc(s, lua_gettop(s) == 1);
...
int uilua_window_set_position(lua_State *s) {
	struct nk_context* ctx = uilua_ctx(s);
	uilua_assert_argc(s, lua_gettop(s) == 3);
	const char *name = luaL_checkstring(s, 1);
	struct nk_vec2 pos;
...
int uilua_layout_row(lua_State *s) {
	struct nk_context* ctx = uilua_ctx(s);
	const int argc = lua_gettop(s);
	uilua_assert_argc(s, argc >= 3 && argc <= 4);
	...
	if (useRatios) {
		...
		uilua_assert(s, cols < lengthof(floatsArray), "Overflow for ratios: '%s'");
...
int uilua_layout_space_to_screen(lua_State *s) {
	struct nk_context* ctx = uilua_ctx(s);
	uilua_assert_argc(s, lua_gettop(s) == 2);
...
int uilua_tree_push(lua_State *s) {
	struct nk_context* ctx = uilua_ctx(s);
	const int argc = lua_gettop(s);
	uilua_assert_argc(s, argc >= 2 && argc <= 4);
	...
	int open = 0;
	if (useImage) {
		open = nk_tree_image_push_hashed(ctx, type, image, title, state, title, strlen(title), id);
	} else {
		open = nk_tree_push_hashed(ctx, type, title, state, title, strlen(title), id);
	}
	lua_pushboolean(s, open);
	return 1;
}
...
int uilua_radio(lua_State *s) {
	struct nk_context* ctx = uilua_ctx(s);
	const int argc = lua_gettop(s);
	uilua_assert_argc(s, argc == 2 || argc == 3);
	const char *name = luaL_checkstring(s, 1);
	const char *text;
	if (argc == 3) {
...
int uilua_property(lua_State *s) {
	...
	if (lua_isnumber(s, 3)) {
		...
		clua_typerror(s, 3, "number or table");
	}
	return 1;
}
...
int uilua_edit(lua_State *s) {
	...
	const char *value = lua_tostring(s, -1);
	constexpr size_t NK_LOVE_EDIT_BUFFER_LEN = 1024;
	char editBuffer[NK_LOVE_EDIT_BUFFER_LEN];
	const size_t len = NK_CLAMP(0, (int)strlen(value), (int)(NK_LOVE_EDIT_BUFFER_LEN - 1));
	memcpy(editBuffer, value, len);
	editBuffer[len] = '\0';
	const nk_flags event = nk_edit_string_zero_terminated(ctx, flags, editBuffer, NK_LOVE_EDIT_BUFFER_LEN - 1, nk_filter_default);
...
int uilua_combobox(lua_State *s) {
	struct nk_context* ctx = uilua_ctx(s);
	const int argc = lua_gettop(s);
	uilua_assert_argc(s, argc >= 2 && argc <= 5);
	if (!lua_istable(s, 2)) {
		clua_typerror(s, 2, "table");
	}
	constexpr int NK_LOVE_COMBOBOX_MAX_ITEMS = 1024;
	const char *combobox_items[NK_LOVE_COMBOBOX_MAX_ITEMS];
	int i;
	for (i = 0; i < NK_LOVE_COMBOBOX_MAX_ITEMS && lua_checkstack(s, 4); ++i) {
		lua_rawgeti(s, 2, i + 1);
		if (lua_isstring(s, -1)) {
...
int uilua_combobox_begin(lua_State *s) {
	struct nk_context* ctx = uilua_ctx(s);
	const int argc = lua_gettop(s);
	uilua_assert_argc(s, argc >= 1 && argc <= 4);
	...
	if (text != NULL) {
		if (useColor) {
			uilua_assert(s, false, "%s: color comboboxes can't have titles");
		} else if (symbol != NK_SYMBOL_NONE) {
		...
		if (useColor) {
			...
			uilua_assert(s, false, "%s: must specify color, symbol, image, and/or title");
		}
	}
	lua_pushboolean(s, open);
	return 1;
...
int uilua_menu_begin(lua_State *s) {
	const int argc = lua_gettop(s);
	uilua_assert_argc(s, argc >= 4 && argc <= 5);
	struct nk_context* ctx = uilua_ctx(s);
	const char *text = luaL_checkstring(s, 1);
	nk_symbol_type symbol = NK_SYMBOL_NONE;
	struct nk_image image;
