...
static bool materialEquals(const palette::Material &a, const palette::Material &b,
						   const core::Buffer<palette::MaterialProperty> &ignoredMaterials, bool ignoreType) {
	if (ignoredMaterials.empty() && !ignoreType) {
		return a == b;
	}
	...
	for (int i = 0; i < (int)palette::MaterialMax - 1; ++i) {
		...
		if (!glm::epsilonEqual(a.value((palette::MaterialProperty)(i + 1)), b.value((palette::MaterialProperty)(i + 1)),
							   glm::epsilon<float>())) {
			return false;
		}
	}
...
void materialComparator(const palette::Palette &pal1, const palette::Palette &pal2,
	...
	for (int i = 0; i < pal2.colorCount(); ++i) {
		...
		for (int j = 0; j < pal1.colorCount(); ++j) {
			...
			if (materialEquals(pal1Mat, pal2Mat, ignoredMaterials, ignoreType)) {
...
void materialComparator(const scenegraph::SceneGraph &graph1, const scenegraph::SceneGraph &graph2,
	...
	for (auto iter = graph1.beginModel(), iter2 = graph2.beginModel(); iter != graph1.end(); ++iter, ++iter2) {
		...
		materialComparator(graph1Pal, graph2Pal, ignoredMaterials, ignoreType);
