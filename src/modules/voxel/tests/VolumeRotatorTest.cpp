...
protected:
	...
	void rotateY90Steps(int degree) {
		...
		EXPECT_TRUE(smallVolume.setVoxel(pos.x, pos.y++, pos.z, createVoxel(voxel::VoxelType::Rock1)));
		...
		EXPECT_EQ(voxel::VoxelType::Rock2, rotated->getVoxel(rotPos.x, rotPos.y++, rotPos.z).getMaterial());
		EXPECT_EQ(voxel::VoxelType::Rock3, rotated->getVoxel(rotPos.x, rotPos.y++, rotPos.z).getMaterial());
	}
};
