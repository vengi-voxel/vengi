...
TEST_F(VoxelUtilTest, testFillHollow3x3Center) {
	...
	const voxel::Voxel borderVoxel = voxel::createVoxel(1);
	...
	const voxel::Voxel fillVoxel = voxel::createVoxel(2);
...
TEST_F(VoxelUtilTest, testExtrudePlanePositiveY) {
	voxel::Region region(0, 2);
	voxel::RawVolume v(region);
	const voxel::Voxel groundVoxel = voxel::createVoxel(2);
	const voxel::Voxel newPlaneVoxel = voxel::createVoxel(3);
	voxel::RawVolumeWrapper wrapper(&v);
	// build an L
...
TEST_F(VoxelUtilTest, testPaintPlanePositiveY) {
	voxel::Region region(0, 2);
	voxel::RawVolume v(region);
	const voxel::Voxel fillVoxel1 = voxel::createVoxel(2);
	const voxel::Voxel fillVoxel2 = voxel::createVoxel(3);
	voxel::RawVolumeWrapper wrapper(&v);
	// build an L
...
TEST_F(VoxelUtilTest, testFillPlaneWithImage) {
	...
	for (int x = 0; x < img->width(); ++x) {
		...
		const voxel::Voxel &voxel = wrapper.voxel(x, 0, 0);
	...
	for (int x = 0; x < img->width(); ++x) {
		const core::RGBA rgba = img->colorAt(x, 0);
		const voxel::Voxel &voxel = wrapper.voxel(x, 0, 1);
		const core::RGBA voxelColor = palLookup.palette().colors[voxel.getColor()];
		EXPECT_EQ(core::Color::getDistance(rgba, voxelColor), 0.0f)
			<< core::Color::print(rgba) << " vs " << core::Color::print(voxelColor) << " (" << (int)voxel.getColor()
			<< ") at " << x << std::endl << v;
