...
AtomicBool::AtomicBool(const AtomicBool& rhs) {
	SDL_SetAtomicInt(&_value, SDL_GetAtomicInt(const_cast<core_atomic*>(&rhs._value)));
}

AtomicBool::operator bool() const {
	return SDL_GetAtomicInt(const_cast<core_atomic*>(&_value)) == 1;
}
...
AtomicInt::operator int() const {
	return SDL_GetAtomicInt(const_cast<core_atomic*>(&_value));
}
