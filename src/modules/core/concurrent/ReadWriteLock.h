...
public:
	...
	void lockRead() const core_thread_acquire_shared(_mutex);
...
public:
	inline ScopedReadLock(const ReadWriteLock& lock) core_thread_acquire_shared(_lock) : _lock(lock) {
		_lock.lockRead();
	}
...
private:
	ReadWriteLock& _lock;
public:
	inline ScopedWriteLock(ReadWriteLock& lock) core_thread_acquire(_lock): _lock(lock) {
		_lock.lockWrite();
	}
