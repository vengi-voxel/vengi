...
App::App(const io::FilesystemPtr& filesystem, const core::EventBusPtr& eventBus, uint16_t traceport) :
		_argc(0), _argv(nullptr), _trace(traceport), _curState(AppState::Construct), _nextState(AppState::InvalidAppState), _suspendRequested(false), _deltaFrame(0L), _initTime(0L), _filesystem(filesystem), _eventBus(eventBus) {
	_now = currentMillis();
