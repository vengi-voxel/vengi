...
inline ::std::ostream &operator<<(::std::ostream &os, const color::RGBA &color) {
	return os << color::Color::print(color).c_str();
}
...
inline testing::AssertionResult CmpHelperRGBAEQ(const char *lhs_expression, const char *rhs_expression,
										 ...
										 float max_distance) {
	const float actual_distance = color::Color::getDistance(lhs_value, rhs_value, color::Color::Distance::HSB);
	...
	msg << "\n  " << lhs_expression;
	msg << "\n    Which is: " << color::Color::print(lhs_value, true).c_str();
	msg << "\n  " << rhs_expression;
	msg << "\n    Which is: " << color::Color::print(rhs_value, true).c_str();
