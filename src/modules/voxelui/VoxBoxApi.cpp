...
bool VoxBoxApi::login(const core::String &username, const core::String &password) {
	...
	_refreshToken = json.strVal("refreshToken", "");
...
core::String VoxBoxApi::loggedInUserId() const {
	if (_refreshToken.empty()) {
		return "";
	}
	...
	return payload.strVal("sub", "");
}

VoxBoxState VoxBoxApi::search(const VoxBoxSearchParams &params) const {
	VoxBoxState state;
	state.count = 0;
	core::String url = core::String::format("%s/api/model/search?page=%d&count=%d", BASE_URL, params.page + 1, params.count);
...
bool VoxBoxApi::upload(const io::FilesystemPtr &filesystem, const core::String &voxFilePath,
	...
	io::BufferedReadWriteStream body;

	auto addField = [&body, &boundary](const char *fieldName, const core::String &value) {
		const core::String part = core::String::format("--%s\r\nContent-Disposition: form-data; name=\"%s\"\r\n\r\n%s\r\n",
													   boundary.c_str(), fieldName, value.c_str());
		body.write(part.c_str(), part.size());
	};
	...
	core::String voxHeader = core::String::format(
		"--%s\r\nContent-Disposition: form-data; name=\"vox\"; filename=\"%s\"\r\nContent-Type: application/octet-stream\r\n\r\n",
		boundary.c_str(), voxFilename.c_str());
	body.write(voxHeader.c_str(), voxHeader.size());
	body.write(voxBuf, voxSize);
	body.write("\r\n", 2);
	...
	body.write(coverHeader.c_str(), coverHeader.size());
	body.write(coverBuf, coverSize);
	body.write("\r\n", 2);

	const core::String closing = core::String::format("--%s--\r\n", boundary.c_str());
	body.write(closing.c_str(), closing.size());

	body.seek(0);
	core::String bodyStr;
	bodyStr.append((const char *)body.getBuffer(), (int)body.size());
