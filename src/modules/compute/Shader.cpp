...
core::String Shader::getSource(const core::String& buffer, bool finalize, core::List<core::String>* includedFiles) const {
	...
	const core::Pair<core::String, bool>& includeFirst = util::handleIncludes(_name, buffer, includeDirs, includedFiles);
	src += includeFirst.first;
	int level = 0;
	while (core::string::contains(src, "#include")) {
		const core::Pair<core::String, bool>& includeRecurse = util::handleIncludes(_name, src, includeDirs, includedFiles);
