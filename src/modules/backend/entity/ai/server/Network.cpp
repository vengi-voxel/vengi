...
void Network::update(int64_t deltaTime) {
	...
	for (ClientSocketsIter i = _clientSockets.begin(); i != _clientSockets.end(); ++clientId) {
		...
		ai::ProtocolMessageFactory& factory = ai::ProtocolMessageFactory::get();
		if (factory.isNewMessageAvailable(client.in)) {
			ai::IProtocolMessage* msg = factory.create(client.in);
			if (!msg) {
				i = closeClient(i);
				continue;
			}
...
bool Network::broadcast(const ai::IProtocolMessage& msg) {
	...
	for (ClientSocketsIter i = _clientSockets.begin(); i != _clientSockets.end(); ++i) {
		...
		ai::IProtocolMessage::addInt(client.out, static_cast<int32_t>(out.size()));
...
bool Network::sendToClient(Client* client, const ai::IProtocolMessage& msg) {
