 ...
 * debug them properly.
 *
 * Each zone has a dedicated GroupMgr instance. For more detailed information, just check out
...
class Zone {
public:
	typedef std::unordered_map<ai::CharacterId, AIPtr> AIMap;
	typedef std::vector<AIPtr> AIScheduleList;
	typedef std::vector<ai::CharacterId> CharacterIdList;
...
public:
	 ...
	 * @note This does not lock the zone for writing but a dedicated schedule lock
	 */
	bool removeAI(const ai::CharacterId& id);
	 ...
	 * already invalid.
	 */
	bool destroyAI(const ai::CharacterId& id);
	 ...
	 */
	AIPtr getAI(ai::CharacterId id) const;
	...
	inline bool executeAsync(ai::CharacterId id, const Func& func) const {
	 ...
	 */
	template<typename Func>
	inline auto execute(ai::CharacterId id, const Func& func) const
		-> typename std::result_of<Func(const AIPtr&)>::type {
	 ...
	 */
	template<typename Func>
	inline auto execute(ai::CharacterId id, Func& func)
		-> typename std::result_of<Func(const AIPtr&)>::type {
		return execute(getAI(id), func);
	}
