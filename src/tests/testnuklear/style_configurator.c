...
style_button(struct nk_context* ctx, struct nk_style_button* out_style, struct nk_style_button** duplicate_styles, int n_dups)
{
	...
	static nk_flags aligns[] = {
		NK_TEXT_LEFT,
		NK_TEXT_CENTERED,
		NK_TEXT_RIGHT,
		NK_TEXT_ALIGN_TOP|NK_TEXT_ALIGN_LEFT,
		NK_TEXT_ALIGN_TOP|NK_TEXT_ALIGN_CENTERED,
		NK_TEXT_ALIGN_TOP|NK_TEXT_ALIGN_RIGHT,
		NK_TEXT_ALIGN_BOTTOM|NK_TEXT_ALIGN_LEFT,
...
style_configurator(struct nk_context *ctx, struct nk_color color_table[NK_COLOR_COUNT])
{
	...
	if (nk_begin(ctx, "Configurator", nk_rect(980, 440, 400, 600), window_flags))
	{
		...
		if (nk_button_label(ctx, "Reset all styles to defaults")) {
			memcpy(color_table, nkc_get_default_color_style(nullptr), NK_COLOR_COUNT * sizeof(struct nk_color));
