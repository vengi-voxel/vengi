	...
	if (UNITTESTS)
		...
		if (USE_SANITIZERS AND LINUX)
			...
			if (EXISTS ${_lsan_suppressions})
				...
				list(APPEND _env_vars "${_lsan_env}")
			endif()
		endif()
		# Work around SDL3 Wayland OpenGL context cleanup crashes on Linux
		# and nvidia dbus assertion failures during atexit cleanup
		if (LINUX)
			list(APPEND _env_vars "SDL_VIDEO_DRIVER=x11")
			list(APPEND _env_vars "DBUS_FATAL_WARNINGS=0")
			set_property(TEST ${name} APPEND PROPERTY ENVIRONMENT "SDL_VIDEO_DRIVER=x11")
			set_property(TEST ${name} APPEND PROPERTY ENVIRONMENT "DBUS_FATAL_WARNINGS=0")
		endif()

		if (_env_vars)
			add_custom_target(${name}-run COMMAND ${CMAKE_COMMAND} -E env ${_env_vars} $<TARGET_FILE:${name}> --gtest_output=xml:${CMAKE_BINARY_DIR}/${name}.xml DEPENDS ${name} USES_TERMINAL WORKING_DIRECTORY "${_gtest_working_dir}")
		else()
